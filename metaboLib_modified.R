...
evalPCA <- function(matrix,
    ...
    if(plot == TRUE) {        
        colours_classes <- colours_classes <- c("#ff00ff", "#ff0000", "#ff8000", "#ffff00",
                                                "#bfff00", "#00ff80", "#00ffff", "#000000",
                                                "#0040ff", "#8000ff","#bf00ff")
        ...
        if(annotate.scores == TRUE) {
            ...
            print(
                ...
                geom_text_repel(size= 2.4, point.padding = 0.2, data = as.data.frame(pca$x[,1:2]),
...

{
    ...
    if(type == "pooled") {
        ...
        qc.factor <- as.data.frame(apply(qc, 2, nom.factor))
