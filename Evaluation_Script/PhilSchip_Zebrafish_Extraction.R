...
polarity <- "positive"   ## used for some result annotation (positive and negative supported)
...
if (length(matrix.sig) > 1) {
  ...
  tsne2 <- Rtsne(matrix.sig, dim = 2, perplexity = tSNE.perplexity, epoch = 100,
...
p.peaknumber <- ggplot(peaknumber, aes(x = factor(extraction, level = peaknumber_order), y = sum)) +
  ...
  stat_compare_means(label = "p.signif", method = "t.test", ref.group = "Extraction_I") +
...
p.peaknumber_homogenization <- ggplot(peaknumber, aes(x = factor(extraction, level = peaknumber_order), y = sum)) +
  geom_boxplot(aes(group = extraction)) +
  stat_compare_means(comparisons = list(c("Extraction_III", "Extraction_III_B")), label = "p.signif", method = "t.test", ref.group = "Extraction_III", color="#ff0000") +
  stat_compare_means(comparisons = list(c("Extraction_IV", "Extraction_IV_B")), label = "p.signif", method = "t.test", ref.group = "Extraction_IV", color="#ffff00") +
  xlab("Extraction Method") +
  ylab("Number of Features") +
  theme_bw()
...
rownames(Peaknumber_Results) <- rownames(peaknumber)
...
if (Target_eval == "IS") {
  ...
  exceldata = read_excel("IS_Inclusion.xlsx") 
}
...
for(i in 1:Targetlength){
  ...
  if(length(is.Target) >0){
    ...
    ## Plots the EIC of each target in a separate PDF datafile
    groupid <- c(is.Target)
